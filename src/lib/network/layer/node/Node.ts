...
export default class Node extends NetworkElement {
    ...
    constructor(
        ...
        lastLayer: Layer | undefined, weightValues: number[] | null,
        nodeIndex: number, type: string
    ) {
        super(parent);
    ...
    derivativeActivationFunction(value: number): number {
        if (this.layerType === "output") { // Use absolute error for output layer
            return 1;
        } else {
    ...
    connectLayer(lastLayer: Layer, weightValues: number[], nodeIndex: number) {
        ...
        for (let i = 0; i < lastNodes.length; i++) {
            ...
            const weight = new Weights(this.networkConfig, lastNode.self, this.self, weightValues[i], nodeIndex);
